...
public class HeMultiplyFunctionExtensionTestCase {
...
//        homomorphicEncDecService.generateKeys("/home/arosha/helib-keys", 1201, 1, 2, 15, 64, 1, 80, 0);
        homomorphicEncDecService.generateKeys("/home/arosha/helib-keys", 1201, 1, 8, 2, 64, 0, 80, 0);
//        homomorphicEncDecService.generateKeys("/home/arosha/helib-keys", 101, 1, 8, 2, 64, 0, 80, 0);
    ...
    @Test
    public void testMultiplyFunctionExtension() throws InterruptedException {
        ...
        String query = ("@info(name = 'query1') " + "from inputStream "
                + "select timestampList, he:multiply(valueList, 3L) as withBonusValueList, size "
            ...
            public void receive(long timeStamp, Event[] inEventsComposite, Event[] removeEventsComposite) {
                ...
                for (Event inEventComposite : inEventsComposite) {
                    ...
                    for (Event inEvent : decryptedEvents) {
                        ...
                        if (count.get() == 1) {
                            Assert.assertEquals(15L, Long.parseLong(String.valueOf(inEvent.getData(1))));
                        }
                        if (count.get() == 2) {
                            Assert.assertEquals(18L, Long.parseLong(String.valueOf(inEvent.getData(1))));
                        }
                        if (count.get() == 3) {
                            Assert.assertEquals(21L, Long.parseLong(String.valueOf(inEvent.getData(1))));
                        }
                        if (count.get() == 4) {
                            Assert.assertEquals(24L, Long.parseLong(String.valueOf(inEvent.getData(1))));
                        }
                        if (count.get() == 5) {
                            Assert.assertEquals(27L, Long.parseLong(String.valueOf(inEvent.getData(1))));
                        }
                    }
                }
            }
        ...
        Object[] event1 = {System.currentTimeMillis(), 5};
        Object[] event2 = {System.currentTimeMillis(), 6};
        Object[] event3 = {System.currentTimeMillis(), 7};
        Object[] event4 = {System.currentTimeMillis(), 8};
        Object[] event5 = {System.currentTimeMillis(), 9};
